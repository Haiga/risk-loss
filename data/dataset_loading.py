...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        """
        x, y, yp_baseline, indices = sample
        if yp_baseline is None:
            return torch.from_numpy(x).type(torch.float32), torch.from_numpy(y).type(torch.float32), torch.tensor([]), torch.from_numpy(indices).type(torch.long)
        return torch.from_numpy(x).type(torch.float32), torch.from_numpy(y).type(torch.float32), torch.from_numpy(yp_baseline).type(torch.float32), torch.from_numpy(indices).type(torch.long)
...
class FixLength(object):
    ...
    def __call__(self, sample):
        ...
        if sample_size < self.dim_given:  # when expected dimension is larger than number of observation in instance do the padding
            fixed_len_x, fixed_len_y, fixed_len_yp, indices = self._pad(sample, sample_size)
        else:  # otherwise do the sampling
            fixed_len_x, fixed_len_y, fixed_len_yp, indices = self._sample(sample, sample_size)

        return fixed_len_x, fixed_len_y, fixed_len_yp, indices
    ...
    def _pad(self, sample, sample_size):
        ...
        if sample[2] is None:
            ...
            fixed_len_yp = np.pad(sample[2], ((0, self.dim_given - sample_size), (0, 0)), "constant")
...
def get_baselines(baselines_file_path):
    if not os.path.isfile(baselines_file_path):
        return None
    with open(baselines_file_path) as bf:
        current = 1
        all_baselines = []
        current_query = []
        for line in bf:
            splitted_line = line.strip().split(',')
            current_query = np.array([float(v) for v in splitted_line])
            all_baselines.append(current_query)
            # splitted_line = line.strip().split(',')
            ...
            #     current_query = []
            # else:
            #     current_query.append([float(v) for v in splitted_line[1:]])
            # current = int(splitted_line[0])
        all_baselines.append(current_query)
...
class LibSVMDataset(Dataset):
    ...
    """
    def __init__(self, X, y, baselines_predicts, query_ids, transform=None):
        """
        ...
        if not baselines_predicts is None:
            self.baselines_predicts_by_qid = np.split(baselines_predicts, groups)[:-1]
        else:
            self.baselines_predicts_by_qid = None
        # self.baselines_predicts_by_qid = baselines_predicts
    ...
    @classmethod
    def from_svm_file(cls, svm_file_path, path, transform=None):
        """
        ...
        baselines_predicts = get_baselines(path.replace("Norm", "baseline.Norm"))
        ...
        return cls(x, y, baselines_predicts, query_ids, transform)
    ...
    def __getitem__(self, idx):
        ...
        X = self.X_by_qid[idx]
        y = self.y_by_qid[idx]
        if not self.baselines_predicts_by_qid is None:
            yp_baseline = self.baselines_predicts_by_qid[idx]
        else:
            yp_baseline = None
        sample = X, y, yp_baseline
...
def load_libsvm_role(input_path: str, role: str) -> LibSVMDataset:
    ...
    """
    path = os.path.join(input_path, "Norm.{}.txt".format(role))
    logger.info("will load {} data from {}".format(role, path))
    with open_local_or_gs(path, "rb") as input_stream:
        ds = LibSVMDataset.from_svm_file(input_stream, path)
