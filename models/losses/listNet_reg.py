...
def geoRiskListnetLoss2(y_predicted, y_baselines, y_true, alpha=2, return_strategy=2, ob=0, corr=0):
    device = y_predicted.device
...
def geoRiskSpearmanLoss2(y_predicted, y_baselines, y_true, alpha=2, return_strategy=2, ob=1, corr=0):
    device = y_predicted.device
...
def listNetreg(y_pred, y1, y_true, eps=DEFAULT_EPS, padded_value_indicator=PADDED_Y_VALUE, wr=1, ws=0):
    """
    ...
    if ws == 0:
        ...
        return loss + wr * geoRiskListnetLoss2(y_pred, y1, y_true)
    elif ws == 2:
        return loss + wr * geoRiskSpearmanLoss2(y_pred, y1, y_true)
    return loss
