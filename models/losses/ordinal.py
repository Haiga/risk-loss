...
def ordinal(y_pred, y1, y_true, n=5, normalized=1, padded_value_indicator=PADDED_Y_VALUE):
    """
    Ordinal loss.
    ...
    y_pred = y_pred.clone()
    if normalized:
        y_true = with_ordinals(n * y_true.clone(), n)
    else:
...
def ordinalmulti(y_pred, y1, y_true, n=5, normalized=1, padded_value_indicator=PADDED_Y_VALUE):
    soma = 0
    if isinstance(y1, list):

        for i in y1:
            soma += ordinal(i, i, y_true, n, normalized, padded_value_indicator)
            # i, y_true
    else:
        for i in range(y1.shape[2]):
            soma += ordinal(y1[:, :, i], y1[:, :, i], y_true, n, normalized, padded_value_indicator)
